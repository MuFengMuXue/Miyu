...
pub fn register_readonly(registry: &mut ToolRegistry) {
    ...
    registry.register(ToolSpec::new(
        "check_os_info",
        t("Check basic read-only OS, shell, desktop session, kernel, host, and package-manager context. For troubleshooting a concrete local issue, prefer check_issue or deep_diagnose.", "查看只读基础系统信息，包括 OS、shell、桌面会话、内核、主机和包管理器上下文。排查具体本机问题时优先使用 check_issue 或 deep_diagnose。"),
...
fn check_os_info() -> Result<String> {
    ...
    Ok(serde_json::to_string_pretty(&json!({
        ...
        "notes": [
            ...
            "This only reports basic OS context. For a concrete local issue, use check_issue or deep_diagnose."
        ],
    }))?)
}
